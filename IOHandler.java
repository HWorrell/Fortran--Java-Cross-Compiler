...
public class IOHandler {
  ...
  public String printFunction(ArrayList<String> s, ArrayList<String> integerArrays, ArrayList<String> realArrays, ArrayList<String> functionNames) {
    ...
    for (int i = 2; i < s.size(); i++) {
      ...
      if (s.get(i).charAt(s.get(i).length() - 1) == ',')
      {
        ...
        {
        //Add token followed by a whitespace
        if (s.get(i).indexOf("(") > 0) {
          if (!functionNames.contains(s.get(i).substring(0, s.get(i).indexOf("("))) && (integerArrays.contains(s.get(i).substring(0, s.get(i).indexOf('('))) || realArrays.contains(s.get(i).substring(0, s.get(i).indexOf('('))))) {
            temp = s.get(i).replace("(", "[");
            temp = temp.replace(")", "-1]");
            javaLine += temp + " ";
          }
        }
