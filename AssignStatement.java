    ...
    for (int i = 0; i < globals.size(); i++)
    {
      for (int j = 0; j < s.length; j++)
      {
        for (int k = 0; k < integer.size(); k++)
        {
          ...
          if (s[j].replaceFirst(",","").equalsIgnoreCase(globals.get(i)) && s[j].contains(integer.get(k)))
          {
            ...
            if (temp >= 0)
            {
              build.replace(temp, temp+j+globals.get(i).length()-1, "");
              j++;
              k++;
            }
            break;
    ...
    for (int i = 0; i < globals.size(); i++)
    {
      for (int j = 0; j < s.length; j++)
      {
        for (int k = 0; k < real.size(); k++)
        {
          ...
          //System.out.println("S IS :::::" +s[j]);

          if (s[j].replaceFirst(",","").equalsIgnoreCase(globals.get(i)) && s[j].contains((real.get(k))))
          {
    ...
    for (int i = 0; i < globals.size(); i++)
    {
      ...
      for (int j = 0; j < s.length; j++)
      {
        for (int k = 0; k < logical.size(); k++)
        {
          ...
          if (s[j].replaceFirst(",","").equalsIgnoreCase(globals.get(i)) && s[j].contains(logical.get(k)))
          {
            System.out.println("WER:LKJWERJ :: ::: " + s[j].toString());
            ...
            if (temp <= 0)
            {
              build.replace(temp, temp+s[j].length()+9, "");
              break;
            }
      ...
      System.out.println ("MORE GLOBALS" + globals);
    }
